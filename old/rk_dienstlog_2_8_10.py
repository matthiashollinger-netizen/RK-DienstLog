...
PORTAL_BASE_URL = "https://portal.s.roteskreuz.at/"
PORTAL_REPORTS_URL = "https://portal.s.roteskreuz.at/home.php"


CHANGELOG_TEXT = """RK DienstLog – Changelog

Version 2.8.10
- Portal-Import verbessert.
- Browser öffnet direkt das RK-Portal.
- Button „Zu Reports öffnen“ ergänzt.
- Erkennung für falsche Seite, Login-Seite und fehlende Tabelle ergänzt.
- Fehlermeldung bei fehlendem Playwright verbessert.
...
def inspect_portal_page(page):
    """Prüft grob, ob die aktuelle Portal-Seite importierbar aussieht."""
    try:
        url = page.url or ""
        html = page.content()
        ...
        if "portal.s.roteskreuz.at" not in url:
            return False, "Der Browser ist nicht auf portal.s.roteskreuz.at geöffnet."
        ...
        has_stats_table_words = all(word in html_lower for word in table_indicators)

        # Wenn Login-Wörter vorkommen und keine Statistik-Tabelle sichtbar ist, wahrscheinlich Login-Seite
        if any(word in html_lower for word in login_indicators) and not has_stats_table_words:
            return False, "Du bist vermutlich noch nicht eingeloggt. Bitte im Browser einloggen und danach erneut importieren."

        if "reports" not in html_lower and "statistik" not in html_lower and not has_stats_table_words:
            return False, (
                "Die aktuelle Seite sieht nicht nach Reports / Statistik aus. "
                "Bitte im Portal auf Reports → bestätigte wechseln."
            )

        if not has_stats_table_words:
            return False, (
                "Keine passende Statistik-Tabelle gefunden. "
                "Bitte prüfe, ob du auf Reports → bestätigte bist und ein Jahr ausgewählt hast."
            )

        if "bestätigte" not in html_lower and "bestaetigte" not in html_lower and "bestätigt" not in html_lower:
            # Nicht hart blockieren, weil manche Seiten den Tab-Namen nicht im HTML haben.
            return True, "Tabelle erkannt. Hinweis: Der Tab „bestätigte“ konnte nicht eindeutig erkannt werden."

        return True, "Portal-Seite sieht importierbar aus."

    except Exception as e:
        return False, f"Portal-Seite konnte nicht geprüft werden: {e}"
...
class PortalImportDialog(ctk.CTkToplevel):
    def __init__(self, parent, callback):
        ...
        info = (
            ...
            "4. Danach „Aktuelle Portal-Seite importieren“ klicken\n\n"
            "Passwörter werden nicht gespeichert."
        )
        ctk.CTkLabel(
            self,
        ...
        ctk.CTkButton(
            btns,
            text="Seite prüfen",
            command=self.check_current_page
        ...
        ctk.CTkButton(
            ...
            fg_color="#555B66",
            hover_color="#464B54"
        ).grid(row=0, column=5, padx=(8, 0), sticky="e")
        ...
        self.write_log(f"Portal-URL: {PORTAL_BASE_URL}")
    ...
    def show_playwright_missing(self):
        top = ctk.CTkToplevel(self)
        top.title("Playwright fehlt")
        ...
        ctk.CTkLabel(
            frame,
            text="Portal-Import benötigt Playwright",
            font=ctk.CTkFont(size=21, weight="bold")
        ).pack(anchor="w", padx=18, pady=(18, 8))

        text = (
            "Für den Portal-Import muss Playwright installiert und Chromium eingerichtet sein.\n\n"
            "Im Terminal / CMD ausführen:\n\n"
            "pip install playwright\n"
            "python -m playwright install chromium\n\n"
            "Danach die App neu bauen.\n\n"
            "Hinweis: Für eine fertige Standalone-Version müssen wir Playwright/Chromium "
            "beim Build zusätzlich berücksichtigen."
        )

        box = ctk.CTkTextbox(frame, height=210, font=("Consolas", 13), wrap="word")
        box.pack(fill="both", expand=True, padx=18, pady=(4, 14))
        box.insert("1.0", text)
        box.configure(state="disabled")

        ctk.CTkButton(frame, text="OK", command=top.destroy, width=140).pack(pady=(0, 18))

    def open_browser(self):
        if sync_playwright is None:
            self.show_playwright_missing()
            return

        try:
            if self.browser is not None and self.page is not None:
                self.page.bring_to_front()
                self.write_log("Browser ist bereits geöffnet.")
                return
            ...
            self.page.goto(PORTAL_BASE_URL, wait_until="domcontentloaded", timeout=30000)
            self.page.bring_to_front()

            self.status.configure(text="Browser geöffnet – bitte im Portal einloggen.")
            self.write_log("Browser geöffnet.")
            self.write_log("Bitte einloggen und zu Reports → bestätigte wechseln.")
        except Exception as e:
            self.status.configure(text="Browser konnte nicht gestartet werden.")
            self.write_log(f"Browser-Start fehlgeschlagen: {e}")
    ...
    def open_reports(self):
        ...
        try:
            self.write_log("Öffne Reports/Home-Seite …")
            self.page.goto(PORTAL_REPORTS_URL, wait_until="domcontentloaded", timeout=30000)
            self.page.bring_to_front()
            self.status.configure(text="Reports/Home-Seite geöffnet – bitte Reports → bestätigte auswählen.")
            self.write_log("Seite geöffnet. Bitte im Portal Reports → bestätigte auswählen.")
        except Exception as e:
            self.write_log(f"Reports-Seite konnte nicht geöffnet werden: {e}")
            messagebox.showerror("Seite konnte nicht geöffnet werden", str(e))
    ...
    def check_current_page(self):
        ...
        ok, message = inspect_portal_page(self.page)
        self.write_log(f"Prüfung: {message}")
        self.status.configure(text=message)

        if ok:
            messagebox.showinfo("Seitenprüfung", message)
        else:
            messagebox.showwarning("Seitenprüfung", message)
    ...
    def import_current_page(self):
        ...
        try:
            self.status.configure(text="Prüfe aktuelle Portal-Seite …")
            ok, message = inspect_portal_page(self.page)
            self.write_log(f"Prüfung: {message}")

            if not ok:
                self.status.configure(text="Import nicht möglich.")
                messagebox.showwarning("Import nicht möglich", message)
                return
